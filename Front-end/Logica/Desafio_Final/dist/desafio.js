...
var Cargos;
(function (Cargos) {
    Cargos[Cargos["Prefeitura"] = 1] = "Prefeitura";
    Cargos[Cargos["Vereador"] = 2] = "Vereador";
})(Cargos || (Cargos = {}));
//Array de objetos
let Candidatos = [
...
const inputsEleitor = () => {
    const nome = readline.question("Digite o nome do eleitor: ");
    const cpf = readline.questionInt("Informe o seu CPF: ");
    const apto = readline.keyInYN("Eleitor Apto a votar? "); // forcei boolean
    const dataVoto = null; // Inicialmente null, mas depois recebe Date
    return { nome, cpf, apto, dataVoto };
};
// função para realizar voto ao candidato
const realizarVoto = () => {
    const cpfEleitor = readline.questionFloat("Informe seu CPF para votar: ");
    ...
    const numeroVoto = readline.questionInt("Digite o numero do candidato em quem deseja Votar:\n ");
...
const tratarDados = () => {
    const tipoExclusao = readline.questionInt("Excluir Candidato (1) ou Eleitor (2)? ");
    if (tipoExclusao === 1) {
        console.table(Candidatos);
        const cpfExcluir = readline.questionInt("Informe o CPF do candidato a ser excluido: ");
        ...
        const cpfExcluir = readline.questionInt("Informe o CPF do eleitor a ser excluido: ");
...
const atualizarDados = () => {
    const tipoAtualizacao = readline.questionInt("Atualizar Candidato (1) ou Eleitor (2)? ");
    if (tipoAtualizacao === 1) {
        console.table(Candidatos);
        const cpfCandidato = readline.questionInt("Informe o CPF do candidato a ser atualizado: ");
        const candidato = Candidatos.find(candidatos => candidatos.cpf === cpfCandidato);
        if (candidato) {
            let validacaoCPF = readline.questionInt(`CPF atual: ${candidato.cpf} - Informe o novo CPF:`) || candidato.cpf;
        ...
        console.table(Eleitores);
        const cpfEleitor = readline.questionInt("Informe o CPF do eleitor a ser atualizado: ");
        const eleitor = Eleitores.find(eleitores => eleitores.cpf === cpfEleitor);
        if (eleitor) {
            let validacaoCPF = readline.questionInt("CPF atual:" + eleitor.cpf + "- Informe o novo CPF: ") || eleitor.cpf;
            ...
            eleitor.apto = readline.keyInYN(`Eleitor apto a votar? (Atual: ${eleitor.apto ? "Sim" : "Nao"})`) || eleitor.apto;
